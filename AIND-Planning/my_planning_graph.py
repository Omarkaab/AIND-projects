...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if new_node.prenodes.issubset(self.s_levels[level]):
                #Add parents to new node
                for s_node in self.s_levels[level]:
                    for new_node_precond in new_node.prenodes:
                        if expr (new_node_precond.symbol) == expr(s_node.symbol):
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for effect_a2 in node_a2.action.effect_rem:
            for precond_a1 in node_a1.action.precond_pos:
                if precond_a1 == effect_a2:
                    return True      
        
        for effect_a1 in node_a1.action.effect_rem:
            for precond_a2 in node_a2.action.precond_pos:
                if precond_a2 == effect_a1:
                    return True      

        # IS THIS USELESS?
        for effect_a2 in node_a2.action.effect_add:
            for precond_a1 in node_a1.action.precond_neg:
                if precond_a1 == effect_a2:
                    return True      
        # IS THIS USELESS?
        for effect_a1 in node_a1.action.effect_add:
            for precond_a2 in node_a2.action.precond_neg:
                if precond_a2 == effect_a1:
                    return True             
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for precond_a1 in node_a1.parents:
            for precond_a2 in node_a2.parents:
                if precond_a2.is_mutex(precond_a1):
                    return True
