...
from game_agent import progressive_score
...
def main():
    ...
    CUSTOM_ARGS = {"method": 'alphabeta', 'iterative': True, 'name':'Student'}
    CUSTOM_ARGS_ID = {"method": 'alphabeta', 'iterative': True, 'name':'ID_Improved'}
    ...
    test_agents = [
#                    Agent(CustomPlayer(score_fn=game_agent.agressive_score, **CUSTOM_ARGS), "agressive_score")
#                    ,Agent(CustomPlayer(score_fn=open_move_score, **CUSTOM_ARGS_ID), "ID_Improved")
#                    ,Agent(CustomPlayer(score_fn=game_agent.open_2nd_moves_score, **CUSTOM_ARGS_ID), "open_2nd_moves_score")
                    Agent(CustomPlayer(score_fn=game_agent.as_deep_as_possible_score, **CUSTOM_ARGS_ID), "as_deep_as_possible_score") 
                    ]
...
#        print("Average number of moves per game is: %f"%(n/(7*4*NUM_MATCHES)))
#        n = 0

        print("Average depth is: %f"%(game_agent.depth_cumul/game_agent.number_of_calls))
        game_agent.depth_cumul = 0        
        game_agent.number_of_calls = 0

    print("Test duration was %d"%(t-time.time()))
